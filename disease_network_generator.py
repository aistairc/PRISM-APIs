    ...
    def add(self, doc_name, entities, events):
        ...
        for event in events.values():
            ...
            if trigger.type in REGULATION_EVENT_TYPES:
                antecedents = [arg for arg in event.args if arg[0] in ANTECEDENT_ROLES] or [(None, None)]
                for antecedent_role, antecedent_id in antecedents:
                    antecedent = antecedent_id and entity_map.get(antecedent_id)
                    antecedent_node_key = antecedent and (antecedent.umls_id or f'"{antecedent.text}"')
                    antecedent_node = antecedent and self.node_key_map[antecedent_node_key]
                    ...
                    for consequent_role, consequent_id in consequents:
                        ...
                        if consequent:
                            ...
                            ] if antecedent_id else []
                            chain_brat_ids = [
                                [event.id],
                                [event.id, consequent_role, consequent_id],
                                [consequent_id],
                            ]
                            chain = [(event.id, antecedent_node, antecedent_brat_ids, chain_brat_ids)]
                            event_descriptor = [True, chain, consequent_node, event.regulation, DIRECT]
                            ...
                            if consequent_event:
                                ...
                                if consequent_type in REGULATION_EVENT_TYPES:
                                    ...
                                    event_descriptor = [True, event.id, antecedent_node, consequent_id, regulation, antecedent_brat_ids, chain_brat_ids]
                                    ...
                                    for sub_consequent_role, sub_consequent_id in sub_consequents:
                                        ...
                                        chain = [(event.id, antecedent_node, antecedent_brat_ids, chain_brat_ids)]
                                        event_descriptor = [True, chain, sub_consequent_node, event.regulation, consequent_type]
                                        regulation_events[event.id].append(event_descriptor)
        ...
        while reg_regulation_events and changed:
            ...
            for reg_event_ix in range(len(reg_regulation_events)):
                ...
                _, event_id, antecedent_node, consequent_id, regulation, antecedent_brat_ids, chain_brat_ids = event_descriptor
                ...
                for consequent_reg_event in consequent_reg_events:
                    changed = True
                    _, chain, sub_consequent_node, sub_regulation, sub_consequent_type = consequent_reg_event
                    new_regulation = Regulation(regulation * sub_regulation)
                    chain = [(event_id, antecedent_node, antecedent_brat_ids, chain_brat_ids)] + chain
                    new_event_descriptor = [True, chain, sub_consequent_node, new_regulation, sub_consequent_type]
                    regulation_events[event_id].append(new_event_descriptor)
        ...
        # XXX it is possible that data is duplicated in case of source being missing:
        # cause in parens, theme as arrow:
        # (null)A1 -> (X)B -> (null)C -> D
        # (null)A2 -> (X)B -> (null)C -> D
        # both generate X -> D edge
        # (null)A1 -> (X)B -> (null)C1 -> D
        # this _should_ generate a second X -> D edge
        # TODO keep list of (event_id, antecedent_node, brat_path) in path instead of last_known_antecedent_node
        # discard till antecedent_node is found, ensure uniqueness on event_ids, then generate brat_path
        ...
        for event_descriptors in regulation_events.values():
            for event_descriptor in event_descriptors:
                ...
                usable, chain, consequent_node, regulation, consequent_type = event_descriptor
                if not usable:
                    # ignore any reg_regulation_events remaining, as they are cyclic
                    continue
                try:
                    chain_start_ix = next(ix for ix, link in enumerate(chain) if link[1])
                except StopIteration:
                    # no antecedent
                    continue
                chain = chain[chain_start_ix:]
                # add antecedent brat ids of the head to all of the chain brat ids
                brat_ids = sum((link[3] for link in chain), chain[0][2])
                antecedent_node = chain[0][1]

                unique_key = tuple(link[0] for link in chain)
                if unique_key in unique_set:
                    # the usable portion of the chain was already seen
                    continue
