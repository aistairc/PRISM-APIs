...
def gen_annotation(fidss, ent_anns, rel_anns, params, result_dir):
    ...
    if not os.path.exists(dir2wr):
        ...
        os.system('rm ' + dir2wr + '*.txt')
        os.system('rm ' + dir2wr + '*.ann')

    os.system('cp ' + params['test_data'] + '*.txt ' + dir2wr)
    ...
    if relations:
        for relation in relations.values():
            relation_annotation = "{}\t{} {}:{} {}:{}".format(
                ...
                relation["left_arg"]["id"].replace("TR", "T"),
                relation["right_arg"]["label"],
                relation["right_arg"]["id"].replace("TR", "T"),
            )
    ...
    if events:
        for event in events.values():
            event_annotation = "{}\t{}:{}".format(
                event["id"], event["trigger_type"], event["trigger_id"].replace("TR", "T")
            )
            for arg in event["args"]:
                event_annotation += " {}:{}".format(arg["role"], arg["id"].replace("TR", "T"))
